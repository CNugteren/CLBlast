...
void CUDAToHost(T** buffer_cuda, std::vector<T>& buffer_host, const size_t size) {
  auto status1 = cudaMemcpy(reinterpret_cast<void*>(buffer_host.data()), reinterpret_cast<void*>(*buffer_cuda),
...
#else
template <typename T>
void CUDAToHost(T**, const std::vector<T>&, const size_t) {}
#endif
...
void HostToCUDA(T** buffer_cuda, std::vector<T>& buffer_host, const size_t size) {
  ...
  auto status2 = cudaMemcpy(reinterpret_cast<void*>(*buffer_cuda), reinterpret_cast<void*>(buffer_host.data()),
...
#else
template <typename T>
void HostToCUDA(T**, const std::vector<T>&, const size_t) {}
#endif
...
void HostToCUDA(const Arguments<U>& args, BuffersCUDA<T>& buffers, BuffersHost<T>& buffers_host,
  ...
  for (auto& name : names) {
    if (name == kBufVecX) {
      HostToCUDA(&buffers.x_vec, buffers_host.x_vec, args.x_size);
    } else if (name == kBufVecY) {
      HostToCUDA(&buffers.y_vec, buffers_host.y_vec, args.y_size);
    } else if (name == kBufMatA) {
      HostToCUDA(&buffers.a_mat, buffers_host.a_mat, args.a_size);
    } else if (name == kBufMatB) {
      HostToCUDA(&buffers.b_mat, buffers_host.b_mat, args.b_size);
    } else if (name == kBufMatC) {
      HostToCUDA(&buffers.c_mat, buffers_host.c_mat, args.c_size);
    } else if (name == kBufMatAP) {
