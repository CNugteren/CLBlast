...
template <typename T>
Xsymm<T>::Xsymm(Queue &queue, EventPointer event, const std::vector<EventPointer>& event_wait_list, const std::string &name):
    Xgemm<T>(queue, event, event_wait_list, name) {
}
