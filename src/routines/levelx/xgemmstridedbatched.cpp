...
template <typename T>
XgemmStridedBatched<T>::XgemmStridedBatched(Queue &queue, EventPointer event, const std::vector<EventPointer>& event_wait_list, const std::string &name):
    Routine(queue, event, event_wait_list, name, {"Copy","Pad","Transpose","Padtranspose","Xgemm","XgemmDirect","GemmRoutine"},
        PrecisionValue<T>(), {}, {
