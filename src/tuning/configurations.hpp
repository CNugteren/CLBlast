...
std::vector<Configuration> SetConfigurations(const Device& device, std::vector<Parameter> parameters,
...
void PopulateConfigurations(const std::vector<Parameter>& parameters, std::vector<size_t> local_size_base,
                            const TransformVector& mul_local_config, const TransformVector& div_local_config,
                            size_t index, const Configuration& config, std::vector<Configuration>& configuration,
                            size_t local_mem_max, const Constraints& constraints,
...
bool ValidConfiguration(const Configuration& config, size_t local_mem_max, const Constraints& constraints,
                        const LocalMemSizeInfo& local_mem_size_info, std::vector<size_t> local_size_base,
                        const TransformVector& mul_local_config, const TransformVector& div_local_config,
                        const std::vector<size_t>& max_work_item_sizes, size_t max_work_group_size);

// Processes multipliers and dividers to obtain the final thread configuration
std::vector<size_t> SetThreadConfiguration(const Configuration& config, std::vector<size_t> base,
