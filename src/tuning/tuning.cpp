...
// Constants holding start and end strings for terminal-output in colour
  ...
  args.platform_id = GetArgument(command_line_args, help, kArgPlatform,
                                 ConvertArgument(std::getenv("CLBLAST_PLATFORM"), size_t{0}));
  args.device_id = GetArgument(command_line_args, help, kArgDevice,
  ...
  for (auto& o : defaults.options) {
    ...
    if (o == kArgBatchCount) {
      args.batch_count =
          GetArgument(command_line_args, help, kArgBatchCount, defaults.default_batch_count);
    }
  }
  ...
  const auto buffer_sizes =
      std::vector<size_t>{settings.size_x + kCanarySize, settings.size_y + kCanarySize,
  ...
  auto configurations =
      SetConfigurations(device, settings.parameters, settings.local_size, settings.mul_local,
  ...
  if (args.fraction != 0.0 && args.fraction != 1.0) {
    ...
    printf("* Exploring a random subset of %s%zu configuration(s)%s\n", kPrintMessage.c_str(),
           configurations.size(), kPrintEnd.c_str());
  }
  ...
  printf(
      "param |      local      |      global     |       compiles |         time | %6s |           "
      " status |\n",
  ...
  for (auto config_id = size_t{0}; config_id < configurations.size(); ++config_id) {
    try {
      ...
      // Runs the kernel
      SetArguments(V, compileInfo.kernel, args, device_buffers);
      const auto time_ms = TimeKernel(args.num_runs, compileInfo.kernel, queue, device,
                                      compileInfo.global, compileInfo.local);
      ...
      for (const auto id : settings.outputs) {
        ...
        for (auto index = size_t{0}; index < buffer_sizes[id]; ++index) {
          const auto diff =
              SquaredDifference(result_buffers[id][index], reference_buffers[id][index]);
          l2_error += diff;
        }
      ...
      const auto status_code = DispatchExceptionCatchAll(true);
      printf("  %scompilation error: %5d%s     |", kPrintError.c_str(),
             static_cast<int>(status_code), kPrintEnd.c_str());
      printf("      - |                 - | <-- skipping\n");
    } catch (...) {
      const auto status_code = DispatchExceptionCatchAll(true);
      if (status_code != StatusCode::kUnknownError) {
        printf("   %serror code %d%s |", kPrintError.c_str(), static_cast<int>(status_code),
               kPrintEnd.c_str());
      }
      printf(" <-- skipping\n");
    }
  ...
  auto comparison = [](const TuningResult& lhs, const TuningResult& rhs) {
...
template void Tuner<half>(int argc, char* argv[], const int V,
                          GetTunerDefaultsFunc GetTunerDefaults,
                          GetTunerSettingsFunc<half> GetTunerSettings,
                          TestValidArgumentsFunc<half> TestValidArguments,
                          SetConstraintsFunc SetConstraints,
                          ComputeLocalMemSizeFunc<half> ComputeLocalMemSize,
                          SetArgumentsFunc<half> SetArguments);
template void Tuner<float>(int argc, char* argv[], const int V,
                           GetTunerDefaultsFunc GetTunerDefaults,
                           GetTunerSettingsFunc<float> GetTunerSettings,
                           TestValidArgumentsFunc<float> TestValidArguments,
                           SetConstraintsFunc SetConstraints,
                           ComputeLocalMemSizeFunc<float> ComputeLocalMemSize,
                           SetArgumentsFunc<float> SetArguments);
template void Tuner<double>(int argc, char* argv[], const int V,
                            GetTunerDefaultsFunc GetTunerDefaults,
                            ...
                            SetArgumentsFunc<double> SetArguments);
template void Tuner<float2>(int argc, char* argv[], const int V,
                            GetTunerDefaultsFunc GetTunerDefaults,
                            ...
                            SetArgumentsFunc<float2> SetArguments);
template void Tuner<double2>(int argc, char* argv[], const int V,
                             GetTunerDefaultsFunc GetTunerDefaults,
