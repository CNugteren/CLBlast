...
int main(void) {
  ...
  for (size_t i = 0; i < n; ++i) {
    host_input[i] = (float)(i % 10);
  }  // staircase modulo 10
  for (size_t i = 0; i < 1; ++i) {
    host_output[i] = 77;
  }  // some temp value to be overwritten later
  ...
  CLBlastStatusCode status = CLBlastiSamax(n, device_output, 0, device_input, 0, 1, &queue, &event);
  ...
  printf(
      "Completed iSAMAX with status %d: array of %zu values with staircases from 0..9 repeated, max at index %u with "
      "value %.0lf\n",
