...
int main(void) {
  ...
  for (size_t i = 0; i < n; ++i) {
    host_a[i] = FloatToHalf(2.2f);
  }
  for (size_t i = 0; i < n; ++i) {
    host_b[i] = FloatToHalf(0.4f);
  }
  printf("Input values at index 0: alpha * a[0] + b[0] == %.3lf * %.3lf + %.3lf\n", HalfToFloat(alpha),
         HalfToFloat(host_a[0]), HalfToFloat(host_b[0]));
  ...
  CLBlastStatusCode status = CLBlastHaxpy(n, alpha, device_a, 0, 1, device_b, 0, 1, &queue, &event);
